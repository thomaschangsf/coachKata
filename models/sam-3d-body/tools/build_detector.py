...
class HumanDetector:
    def __init__(self, name="vitdet", device="cpu", **kwargs):
        # Auto-detect device if "auto" is passed
        if device == "auto":
            try:
        ...
        if name == "vitdet":
            ...
            # Safely move detector to device
            try:
                self.detector = self.detector.to(self.device)
            except (AssertionError, RuntimeError) as e:
                print(f"Warning: Could not move detector to {self.device}: {e}, using CPU")
                self.device = "cpu"
                self.detector = self.detector.to(self.device)
            self.detector.eval()
        elif name == "sam3":
    ...
    def run_human_detection(self, img, **kwargs):
        # Pass device to the detector function if it accepts it
        if hasattr(self.detector_func, '__code__') and 'device' in self.detector_func.__code__.co_varnames:
