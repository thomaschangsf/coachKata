...
if platform.system() == "Darwin":  # macOS
    # On macOS, don't force EGL - let pyrender use the default OpenGL backend
    # This allows it to work in Jupyter notebooks on macOS
...
def setup_sam_3d_body(
    ...
    Args:
        ...
        device: Device to use - "auto" (default), "cuda", or "cpu"
    ...
    if device == "auto" or device is None:
        try:
            # Check if CUDA is available and PyTorch was compiled with CUDA support
            ...
            # PyTorch not compiled with CUDA support or CUDA error
            device = "cpu"
            print(f"CUDA not available ({type(e).__name__}), using CPU")
    elif device == "cuda":
        # User explicitly requested CUDA, check if it's available
        try:
    ...
    if segmentor_name and segmentor_path:
        ...
        try:
            ...
            print(f"Warning: Could not load segmentor ({e}), continuing without it")
    ...
    if fov_name:
        ...
        try:
            ...
            print("  Note: MoGe is optional. Install with: pip install git+https://github.com/microsoft/MoGe.git")
            print("  Continuing without FOV estimator (will use default FOV)")
            fov_estimator = None
        except Exception as e:
            print(f"Warning: Error loading FOV estimator ({e})")
            print("  Continuing without FOV estimator (will use default FOV)")
            fov_estimator = None
