...
# Set OpenGL platform based on OS
# EGL doesn't work on macOS, so use osmesa for headless rendering or let it auto-detect
if "PYOPENGL_PLATFORM" not in os.environ:
    if platform.system() == "Darwin":  # macOS
        # On macOS, don't force EGL - let pyrender use the default OpenGL backend
        # For headless rendering, osmesa can be used but requires installation
        # For now, we'll let it auto-detect which should work in most cases
        pass
    else:
