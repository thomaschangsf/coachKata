...
def recursive_to(x: Any, target: torch.device | str):
    ...
    Args:
        x (Any): Batch of data.
        target (torch.device or str): Target device.
    Returns:
        Batch of data where all tensors are transfered to the target device.
    """
    # Normalize target device - handle CUDA availability
    if isinstance(target, str):
        if target == "cuda":
            ...
            pass  # Keep as "numpy" for special handling
        else:
            # Convert string to device if it's a valid device string
            try:
                target = torch.device(target)
            except (ValueError, RuntimeError):
                # If device string is invalid, default to CPU
                target = "cpu"
    elif isinstance(target, torch.device):
        # If it's a CUDA device but CUDA isn't available, convert to CPU
        if target.type == "cuda":
            try:
                if not torch.cuda.is_available():
    ...
    if isinstance(x, dict):
        ...
        if target == "numpy":
            ...
            try:
                ...
                # If CUDA device fails, fallback to CPU
                if isinstance(target, str) and "cuda" in target.lower():
                    return x.to("cpu")
                elif isinstance(target, torch.device) and target.type == "cuda":
                    return x.to("cpu")
                else:
                    raise e
